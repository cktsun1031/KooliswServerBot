...
module.exports = {
  name: 'sync',
  cooldown: 1000 * 60,
  desc: '與ROBLOX群組實時同步你的身份組和群組等級',
  run: async ({ msg }) => {
    const embed = new MessageEmbed()
    let rank = 0
    let { member } = msg
    if (member.permissions.has('MANAGE_ROLES')) {
      member = msg.mentions.members.first() || member
    }
    const messageProcess = await msg.channel.send({
      content: `<@${msg.author.id}>, 🔍現在正在獲取數據`,
    })
    let desc = '**身份組同步:**'
    let rolesInfo = ''
    ...
    const robloxUserid = await getLinkedUser(member.id, msg.guild.id)
    if (robloxUserid) {
