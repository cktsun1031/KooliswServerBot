...
module.exports = {
  ...
  run: async ({ msg, args }) => {
    ...
    const resn = args.slice(2).join(' ')
      ? args.slice(2).join(' ')
      : '沒有給予原因'
    embed.setTitle('請核實是否進行確定進行強硬封鎖!').addFields(
      {
        name: '目標成員',
    ...
    const int1 = new MessageButton()
      .setStyle('SUCCESS')
      .setLabel('確定')
      .setCustomId('cmdHBanA')
    const int2 = new MessageButton()
      .setStyle('DANGER')
      .setLabel('取消')
      .setCustomId('cmdHBanD')
    ...
    cll.on('collect', async (int) => {
      switch (int.customId) {
        case 'cmdHBanA': {
          ...
          status = true
          await userRes.send({
            content: `你在 **${msg.guild.name}** 被強硬封鎖, **${resn}**`,
          })
          userRes.ban({ day: 7, reason: resn })
          const embed1 = new MessageEmbed()
            .setColor('GREEN')
            .setDescription(
              `${emojis.tick} 已強硬封鎖 **${userRes.user.tag}**, **${resn}**`,
            )
