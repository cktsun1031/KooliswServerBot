...
module.exports = {
  ...
  run: async ({ msg, args }) => {
    ...
    const resn = args.slice(2).join(' ')
      ? args.slice(2).join(' ')
      : '沒有給予原因'
    embed.setTitle('請核實是否進行確定進行踢出!').addFields(
      {
        name: '目標成員',
    ...
    const filter = (ii) => ['cmdKickA', 'cmdKickD'].includes(ii.customId)
    ...
    cll.on('collect', async (int) => {
      switch (int.customId) {
        case 'cmdKickA': {
          ...
          status = true
          await userRes.send({
            content: `你在 **${msg.guild.name}** 被踢出, **${resn}**`,
          })
          userRes.kick({
            reason: resn,
          })
          const embed1 = new MessageEmbed()
            .setColor('GREEN')
            .setDescription(
              `${emojis.tick} 已踢出 **${userRes.user.tag}**, **${resn}**`,
            )
