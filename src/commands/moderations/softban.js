...
module.exports = {
  ...
  run: async ({ msg, args }) => {
    ...
    const resn = args.slice(2).join(' ')
      ? args.slice(2).join(' ')
      : '沒有給予原因'
    embed.setTitle('請核實是否進行確定進行短期封鎖!').addFields(
      {
        name: '目標成員',
    ...
    cll.on('collect', async (int) => {
      switch (int.customId) {
        case 'cmdBanA': {
          ...
          status = true
          await userRes.send({
            content: `你在 **${msg.guild.name}** 被短期封鎖, **${resn}**`,
          })
          const usr = await userRes.ban({ day: 2, reason: resn })
          msg.guild.members.unban(usr, { reason: '短期封鎖' }).catch(() => null)
          const embed1 = new MessageEmbed()
            .setColor('GREEN')
            .setDescription(
              `${emojis.tick} 已短期封鎖 **${userRes.user.tag}**, **${resn}**`,
            )
