...
module.exports = {
  ...
  async run(inter) {
    ...
    let rank = 0
    const { member, user, guild } = inter
    if (!member) await guild.members.fetch(user.id)
    ...
    await inter.reply({
      ephemeral: true,
      content: `<@${member.id}>, 🔍現在正在獲取數據`,
    })
    let desc = '**身份組同步:**'
    let rolesInfo = ''
    for (let index = 0, l = shouldBeAddedToRole.length; index < l; index++) {
      const targetRole = shouldBeAddedToRole[index]
      if (!member.roles.cache.has(targetRole[0])) continue
      if (!member.roles.cache.has(targetRole[1])) {
    ...
    const robloxUserid = await getLinkedUser(member.id, inter.guild.id)
    if (robloxUserid) {
